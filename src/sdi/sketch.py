...
def make_count_sketch(
    *,
    ...
    h = torch.randint(0, m, (d,), device=device, generator=generator, dtype=torch.int64)
...
def make_tensor_sketch(
    *,
    ...
    h_out = torch.randint(
        0, m, (d_out,), device=device, generator=generator, dtype=torch.int64
    )
    s_out = _rand_signs(size=d_out, device=device, generator=generator)
    h_in = torch.randint(
        0, m, (d_in,), device=device, generator=generator, dtype=torch.int64
    )
